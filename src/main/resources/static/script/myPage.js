...
$('#toggle').on('change', function() {
    const clientCode = $('#toggle').val();
    handleChange($('#toggle')[0], clientCode)
});


function handleChange(checkbox, clientCode) {
    if (checkbox.checked) {
        requestPermission(clientCode);
    } else {
        deleteToken(clientCode)
    }
}
    ...
    Notification.requestPermission().then((permission) => {
        if(permission === "granted") {
            ...
            getToken(messaging, {
                ...
                if(currentToken) {
                    saveToken(currentToken, clientCode);
                } else {
                    console.log("ㅜㅜ");
                }
            });
...
function saveToken(currentToken, clientCode) {
    $.ajax({
            "url": "/saveToken",
            "method": "POST",
            "timeout": 0,
...
function deleteToken(clientCode) {
    $.ajax({
            "url": `/deleteToken?clientCode=${clientCode}`,
            "method": "POST",
            "timeout": 0,
